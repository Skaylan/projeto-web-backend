...
class Movie(Base): 
    ...
    director: Mapped[str] = mapped_column(String(100), unique=True, nullable=False)
    rating: Mapped[Double] = mapped_column(Double, unique=True, nullable=False) 
    banner_img_id: Mapped[str] = mapped_column(Text, unique=True, nullable=False) 
    launch_date: Mapped[str] = mapped_column(String(45), unique=True, nullable=False)
    cover_img_id: Mapped[str] = mapped_column(Text, unique=True, nullable=False) 
    producer: Mapped[str] = mapped_column(String(100), unique=True, nullable=False) 
    running_time: Mapped[int] = mapped_column(Integer, unique=True, nullable=False) 

    category_id: Mapped[str] = mapped_column(String, ForeignKey("category.id"), unique=True, nullable=False) 

    def __init__(self, title: str, original_title: str, romanised_original_title: str, description: str, studio: str, director: str,
                 rating: Double, banner_img_id: str, launch_date: str, cover_img_id: str, producer: str, running_time: int,
                 category_id: str
                 ):
        self.title = title
